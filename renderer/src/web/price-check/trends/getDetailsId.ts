...
export function getDetailsId(item: ParsedItem) {
  ...
  if (item.info.refName === "Filled Coffin") {
    ...
    const t = {
      ns: item.info.namespace,
      name: stat.stat.ref.replace(
        "#",
        String(stat.sources[0].contributes?.value),
      ),
...
function getUniqueVariant(item: ParsedItem) {
  ...
  if (uniqueName === "Vessel of Vinktar") {
    ...
    if (hasStat(item, "#% increased Armour, Evasion and Energy Shield")) {
      ...
      hasStat(item, "#% increased Evasion and Energy Shield") &&
      hasStat(item, "+# to maximum Energy Shield")
    ) {
      return "Evasion/ES";
    } else if (
      hasStat(item, "#% increased Evasion and Energy Shield") &&
      hasStat(item, "+# to maximum Life")
    ) {
      return "Evasion/ES/Life";
    } else if (
      hasStat(item, "#% increased Armour and Energy Shield") &&
      hasStat(item, "+# to maximum Energy Shield")
    ) {
      return "Armour/ES";
    } else if (
      ...
      hasStat(item, "#% increased Armour and Evasion") &&
      hasStat(item, "+# to maximum Life")
    ) {
      return "Armour/Evasion";
    } else if (
      hasStat(item, "+# to maximum Energy Shield") &&
      hasStat(item, "#% increased Energy Shield")
    ) {
      return "ES";
    } else if (
      hasStat(item, "#% increased Evasion Rating") &&
      hasStat(item, "+# to maximum Life")
    ) {
      return "Evasion";
    } else if (
      hasStat(item, "#% increased Armour") &&
      hasStat(item, "+# to maximum Life")
    ) {
      return "Armour";
    }
  } else if (
    uniqueName === "Bubonic Trail" ||
    uniqueName === "Lightpoacher" ||
    uniqueName === "Shroud of the Lightless" ||
    uniqueName === "Tombfist"
  ) {
    const sockets = item.statsByType.find(
      (m) => m.type === "explicit" && m.stat.ref === "Has # Abyssal Sockets",
    )!;
    ...
    if (hasStat(item, "Adds # to # Cold Damage to Spells and Attacks")) {
      ...
      hasStat(item, "Adds # to # Lightning Damage to Spells and Attacks")
    ) {
      return "Lightning";
    }
    ...
    if (
      hasStat(
        item,
        "Projectiles from Attacks have #% chance to Maim on Hit while\nyou have a Bestial Minion",
      )
    ) {
      return "Maim";
    } else if (
      hasStat(
        item,
        "Projectiles from Attacks have #% chance to Poison on Hit while\nyou have a Bestial Minion",
      )
    ) {
      return "Poison";
    } else if (
      hasStat(
        item,
        "Projectiles from Attacks have #% chance to inflict Bleeding on Hit while\nyou have a Bestial Minion",
      )
    ) {
      return "Bleeding";
    }
    ...
    const passives = item.statsByType.find(
      (m) => m.stat.ref === "Adds # Small Passive Skills which grant nothing",
    )!;
