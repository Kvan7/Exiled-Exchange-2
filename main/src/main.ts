...
ensureAccessibilityPermission().then(() => {
  app.on("ready", async () => {
...
async function ensureAccessibilityPermission() {
  ...
  const hasPermission = await ensureAccessibilityPermissionOnMac();
  if (!hasPermission) {
...
async function ensureAccessibilityPermissionOnMac(): Promise<boolean> {
