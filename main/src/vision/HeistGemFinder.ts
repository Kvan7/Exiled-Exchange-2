...
export class HeistGemFinder {
  ...
  static async create(binDir: string): Promise<HeistGemFinder> {
    const needleImg = Bmp.decode(
      await fs.readFile(binDir + "/heist-lock.bmp"),
      { toRGBA: true },
    );
  ...
  ocrScreenshot(screenshot: ImageData): OcrResult {
    ...
    const graySize = new cv.Size(
      Math.floor(screenshot.width / scale),
      REFERENCE_HEIGHT,
    );
    ...
    elapsed += timeIt(() => {
      if (scale > 2.1) {
        cv.resize(
          colorMat,
          grayMat,
          new cv.Size(graySize.width * 2, graySize.height * 2),
          0,
          0,
          cv.INTER_LINEAR,
        );
        cv.resize(
          grayMat,
          grayMat,
          new cv.Size(graySize.width, graySize.height),
          0,
          0,
          cv.INTER_LINEAR,
        );
    ...
    matchesMat.delete();
    const clusteredMatches = groupWeightedPoints(
      matches!,
      Math.hypot(needleMat.cols, needleMat.rows),
    );
